import {
    AppointmentState,
} from "@/app/redux/types/AppointmentStateTypes";
...
// const familyRelationMapper = (dataObject: any, relation: string) => {
//     const relationArray = []
//
...
//                     occupation: convertToArray(dataObject[`${relationLowercase}Occupation`])[i],
//                     medicalHistory: {
//                         dm: convertToArray(dataObject[`${relationLowercase}DM`])[i],
//                         epilepsy: convertToArray(dataObject[`${relationLowercase}Epilepsy`])[i],
//                         ht: convertToArray(dataObject[`${relationLowercase}HT`])[i],
//                         cancer: convertToArray(dataObject[`${relationLowercase}Cancer`])[i],
...
//                     },
//                     socialHistory: {
//                         alcoholic: convertToArray(dataObject[`${relationLowercase}Alcoholic`])[i],
//                             smoking: convertToArray(dataObject[`${relationLowercase}Smoking`])[i],
//                     },
//                     surgicalHistory: {
//                         surgery: convertToArray(dataObject[`${relationLowercase}Surgery`])[i],
//                     }
//                 }
//             }
//
//             relationList.push(relationListItem);
//         }
//
//         return relationList;
//     } else {
    ...
    async (dataObject: any, {rejectWithValue}) => {
        ...
        const body = JSON.stringify({
            patient: {
                ...
                foodAllergies: convertToArray(dataObject["patientFoodAllergies"]).length > 0 ?
                    convertToArray(dataObject["patientFoodAllergies"])?.map(
                        (allergy: FormDataEntryValue | null) => ({name: allergy})
                    ) : null
                ,
                drugAllergies: convertToArray(dataObject["patientDrugAllergies"]).length > 0 ?
                    convertToArray(dataObject["patientDrugAllergies"])?.map(
                        (allergy: FormDataEntryValue | null) => ({name: allergy})
                    ) : null
                ,
                // relations: [
                //     dataObject["motherName"] ? familyRelationMapper(dataObject, "Mother") : null,
                //     dataObject["fatherName"] ? familyRelationMapper(dataObject, "Father") : null,
                //     // dataObject["childName"] ? dataObject["childName"].map(() => familyRelationMapper(dataObject, "Child")) : null,
                //     dataObject["childName"] ? convertToArray(familyRelationMapper(dataObject, "Child")) : null
                // ].filter(Boolean),

                relations: familyRelationMapper(dataObject, ["Mother", "Father", "Child"]).filter(Boolean)
            },
            appointmentSymptoms: convertToArray(dataObject["patientSymptoms"]).length > 0 ?
                convertToArray(dataObject["patientSymptoms"])?.map(
                    (symptom: FormDataEntryValue | null, i) => (
                        {
                            symptom: {name: symptom},
                            duration: convertToArray(dataObject["patientSymptomDurations"])[i] ?
            ...
            examinationReport: {
                ...
                firstBloodPressure: convertToArray(dataObject["patientFirstBloodPressure"]).length == 2 ?
                    `${convertToArray(dataObject["patientFirstBloodPressure"])[0]}/${convertToArray(dataObject["patientFirstBloodPressure"])[1]}`
                    : null
                ,
                secondBloodPressure: convertToArray(dataObject["patientSecondBloodPressure"]).length == 2 ?
                    `${convertToArray(dataObject["patientSecondBloodPressure"])[0]}/${convertToArray(dataObject["patientSecondBloodPressure"])[1]}`
                    : null
                ,
                leftEye: convertToArray(dataObject["patientLeftVision"]).length == 2 ?
                    `${convertToArray(dataObject["patientLeftVision"])[0]}/${convertToArray(dataObject["patientLeftVision"])[1]}`
                    : null
                ,
                rightEye: convertToArray(dataObject["patientRightVision"]).length == 2 ?
                    `${convertToArray(dataObject["patientRightVision"])[0]}/${convertToArray(dataObject["patientRightVision"])[1]}`
                    : null
            },
            bloodReport: {
            ...
            diagnoses: convertToArray(dataObject["patientDiagnoses"]).length > 0 ?
