  ...
  async (dataObject: any, { rejectWithValue }) => {
    ...
    const body = JSON.stringify({
      patient: {
        ...
        foodAllergies: [dataObject["patientFoodAllergies"]]?.map(
          (allergy: FormDataEntryValue | null) => ({ name: allergy })
        ),
        drugAllergies: [dataObject["patientDrugAllergies"]]?.map(
          (allergy: FormDataEntryValue | null) => ({ name: allergy })
        ),
        relations: [],
      },
      symptoms: [dataObject["patientSymptoms"]]?.map(
        (symptom: FormDataEntryValue | null) => ({ name: symptom })
      ),
      examinationReport: {
        ...
        firstBloodPressure: 2,
        secondBloodPressure: 2,
        leftEye: 2,
        rightEye: 2,
      },
      bloodReport: {
